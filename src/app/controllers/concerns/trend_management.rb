module TrendManagement
  ...
  def frequent_words(items:)
    ...
    sorted_words = word_count.sort_by { |_, count| -count }.first(100)#単語の頻出度順、大きくするとmeilisearchの回数が増える
    word_usage_count = {}
    sorted_words.to_a.each do |word, _|
      word_usage_count[word] = [items.search(word).count, _]
    end
    word_usage_count.sort_by { |_, counts| -counts[0] }.first(30).to_h#投稿の数順
  end
  def current_trend(limit: 30)
    Rails.logger.info("==#{Rails.application.config.x.last_trend_at.blank?}=====3")
    if Rails.application.config.x.last_trend_at.blank?
      Rails.application.config.x.last_trend_at = nil
    end
    if Rails.application.config.x.last_trend_at && Rails.application.config.x.last_trend_at > Time.now - 60 * 60
